...
SHOWUNAVAIL = False  # hardcoded - removed from settings
...
def resolve(text):
    try:
        if '$LOCALIZE' in text:
            id   = int(re.compile(r'\$LOCALIZE\[(.+?)\]').search(text).group(1))
            text = text.replace('$LOCALIZE[%d]' % id, xbmc.getLocalizedString(id))
            return resolve(text)

        if '$INFO' in text:
            str  = re.compile(r'\$INFO\[(.+?)\]').search(text).group(1)
...
def _shiftUpIndex(index, max, faves, depth=0):
    if depth > max:
        return index  # all items invalid - stay put
    index -= 1
    if index < 0:
        index = max
    cmd = faves[index][2]
    if isValid(cmd):
        return index
    return _shiftUpIndex(index, max, faves, depth + 1)


def _shiftDownIndex(index, max, faves, depth=0):
    if depth > max:
        return index  # all items invalid - stay put
    index += 1
    if index > max:
    ...
    if isValid(cmd):
        return index
    return _shiftDownIndex(index, max, faves, depth + 1)
...
def removeSFOptions(cmd):
    ...
    cmd = re.sub(r'&sf_options=(.+?)_options_sf"\)', '")',               cmd)
    cmd = re.sub(r'&sf_options=(.+?)_options_sf",return\)', '",return)', cmd)
    cmd = re.sub(r'&sf_options=(.+?)_options_sf',    '',                 cmd)
...
def _removeFanart(cmd):
    ...
    cmd = re.sub(r'&sf_fanart=(.+?)_"\)', '")',               cmd)
    cmd = re.sub(r'&sf_fanart=(.+?)_",return\)', '",return)', cmd)
    cmd = re.sub(r'&sf_fanart=(.+?)_',    '',                 cmd)
...
def _getFanart(cmd):
    ...
    try:    return unquote_plus(re.compile(r'sf_fanart=(.+?)_"\)').search(cmd).group(1))
    except: pass
