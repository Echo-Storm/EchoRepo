...
class Main():
	...
	def setother(self):
		...
		# Always call get_fanartbg — the function has its own three-level
		# fallback (cached real image -> per-code folder -> previously cached
		# real image when fanartcode is empty -> Fallback_*.jpg), so the home
		# screen almost never goes black even when fanartcode is missing
		# during a partial weather.json read or a weather_code:null hiccup.
