...
def log(msg, level=0):
	if level == 1:
		xbmc.log(msg=f'[weather.kodiweather]: [W] {msg}', level=xbmc.LOGINFO)
	elif level == 2:
		xbmc.log(msg=f'[weather.kodiweather]: [E] {msg}', level=xbmc.LOGINFO)
	elif level == 3:
		if config.addon.debug:
			xbmc.log(msg=f'[weather.kodiweather]: [D] {msg}', level=xbmc.LOGINFO)
	elif level == 4:
		if config.addon.verbose:
			xbmc.log(msg=f'[weather.kodiweather]: [V] {msg}', level=xbmc.LOGINFO)
	else:
		xbmc.log(msg=f'[weather.kodiweather]: [I] {msg}', level=xbmc.LOGINFO)
...
# Convert ISO 8601 datetime string to localised display string (for NWS alert expiry)
def conv_isodt(isostr):
	try:
		dt_obj = datetime.fromisoformat(isostr).astimezone()
		return dt_obj.strftime(f'{config.kodi.date} {config.kodi.time}')
	except Exception:
		return isostr
...
def getprop(data, map, idx, count):
	...
	if unit == 'round':
		...
		content = config.get_fanart_folder(config.map_wmo.get(f'{content}{isday}'))
	elif unit == 'date':
	...
	# TimeOfDay
		...
		for d in range(0, config.maxdays):
			...
			if not l or not ll:
				for i in [ 'date', 'shortdate', 'weekday', 'weekdayshort', 'condition', 'outlook', 'outlookicon', 'outlookiconwmo', 'temperature', 'maxoutlook', 'maxoutlookicon', 'maxoutlookiconwmo', 'fanartcode', 'fanartcodewmo' ]:
					addprop(f'daily.{idxtod3}.overview.{i}', '')
					addprop(f'daily.{idxtod3}.{i}', '')
					addprop(f'day{idxtod3-1}.{i}', '')
				idxday += 24
				idxtod3 += 1
				continue
			...
			addprop(f'daily.{idxtod3}.overview.fanartcode', config.get_fanart_folder(config.map_wmo.get(f"{code}d")))
			...
			addprop(f'daily.{idxtod3}.fanartcode', config.get_fanart_folder(config.map_wmo.get(f"{code}d")))
			...
			addprop(f'day{idxtod3-1}.fanartcode', config.get_fanart_folder(config.map_wmo.get(f"{code}d")))
			...
			for t in range(0,4):
				...
				# Skip this time slot if no data available (beyond 72-hour window)
				if not l or not ll or not lll:
					idxmid += 6
					continue
				...
				try:
					...
					isday = ll[0] if ll else 1  # Fallback to first value or daytime
				...
				if start:
					...
					addprop(f'timeofday.{idxtod}.fanartcode', config.get_fanart_folder(config.map_wmo.get(f"{code}{isday}")))
				...
				addprop(f'daily.{idxtod2}.{tod.get(t)}.fanartcode', config.get_fanart_folder(config.map_wmo.get(f"{code}{isday}")))
