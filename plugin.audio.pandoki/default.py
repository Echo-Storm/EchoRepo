...
virtualizer = query.get('virtualizer')[0] if query.get('virtualizer') else None
visualizer  = query.get('visualizer')[0]  if query.get('visualizer')  else None
...
if search:
    ...
    Prop('virtualizer', '1')
    Wait('action', 'virtualizer')

elif visualizer:
    Prop('visualizer', '1')
    Wait('action', 'visualizer')
